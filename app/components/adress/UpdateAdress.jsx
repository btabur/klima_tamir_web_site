...
const UpdateAdress = ({setIsShowupdateAdressModal,updateAdres,setIsGetAdress}) => {
  // Form verilerini tutmak için state tanımlayın
  const [formData, setFormData] = useState({
    name:updateAdres.name,
    city: updateAdres.city,
    district: updateAdres.town,
    mah:updateAdres.mah,
  ...
  return (
    <section className='absolute top-0 left-0 w-full h-screen bg-slate-400 bg-opacity-50 flex items-start justify-center'>
        <div className='relative flex flex-col items-center w-1/2 min-w-96 bg-white rounded-lg mt-10 min-h-72 px-10 pt-5'>
              <button onClick={()=> setIsShowupdateAdressModal(false)}
               className='absolute top-2 right-2 text-black text-3xl cursor-pointer'>
              <IoMdCloseCircleOutline />
              </button>
            <h2 className='text-xl w-1/2 shadow-lg text-center rounded-lg font-semibold  mb-3 bg-[#ff8000] text-white'>Adres Güncelle</h2>
            <form className='flex flex-col items-start gap-3 lg:px-10 text-black w-full mt-5' onSubmit={handleSubmit}>
              <div className='flex items-center w-full gap-4'>
                ...
                <input type="text" id='name' className=' border border-black w-full py-2 pl-2 rounded-lg' name='name' onChange={handleChange}  defaultValue={formData.name} />
                </div>
                <div className='relative w-1/2'>
                  <label className='absolute -top-3 left-6 bg-white px-2' htmlFor="city"> Şehir <span className='text-red-600'>*</span></label>
                  <input id='city' className='border border-black w-full py-2 pl-2 rounded-lg' type="text" name="city"  onChange={handleChange} defaultValue={formData.city} />
                </div>
              </div>
              <div className='flex items-center w-full gap-4 mt-4'>
                <div className='relative w-1/2'>
                  <label htmlFor="name" className='absolute -top-3 px-2 left-6 bg-white' >İlçe <span className='text-red-600 '>*</span> </label>
                  <input className='border border-black w-full py-2 pl-2 rounded-lg' type="text" name="district"  onChange={handleChange} defaultValue={formData.district} />
                </div>
                <div className='relative w-1/2'>
                  <label className='absolute -top-3 left-6 bg-white px-2' htmlFor="mah"> Mahalle <span className='text-red-600'>*</span></label>
                  <input id='mah' className='border border-black w-full py-2 pl-2 rounded-lg' type="text" name="mah"  onChange={handleChange} defaultValue={formData.mah} />
                </div>
              </div>
              <div className='flex items-center w-full gap-4 mt-4'>
                <div className='relative w-1/2'>
                    <label className='absolute -top-3 left-6 bg-white px-2' htmlFor="mah"> Adres <span className='text-red-600'>*</span></label>
                    <textarea className='border border-black w-full py-2 pl-2 rounded-lg' type="text" name="address" onChange={handleChange} defaultValue={formData.address} />
                  </div>
                  <div className='relative w-1/2'>
                  <label className='absolute -top-3 left-6 bg-white px-2' htmlFor="mah"> Telefon <span className='text-red-600'>*</span></label>
                  <input className='border border-black w-full py-2 pl-2 rounded-lg' type="phone" name="phone" onChange={handleChange} defaultValue={formData.phone} />
                </div>
                </div>
              ...
              <button type='sumbit' className='mb-5 border py-2 px-4 shadow-xl rounded-lg bg-[#ff8000] text-white font-semibold'>Güncelle</button>
            </form>
        </div>
    </section>
  )
